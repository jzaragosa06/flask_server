...
def forcast_uni(df_arg, lag_value, steps_value, freq, forecast_method='without_refit'):
    ...
    if forecast_method == 'without_refit':
        ...
        forecast_df = pd.DataFrame(data=pred_values, index=pred_indices, columns=['target'])

    return forecast_df


def evaluate_model(df_arg, lag_value, steps_value, freq, forecast_method='without_refit'):
    df = df_arg.copy()
